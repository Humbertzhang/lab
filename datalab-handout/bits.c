...
unsigned float_i2f(int x) {
  ...
  while(1){
    if(x&i) break;
    exp -= 1;     //exp 总共减去第一个1前面的位数
    x <<= 1;      //算出移到第一个1之前需要位移几位
  }                         
  
  exp = ((exp+127) << 23);  //把小数点移到第一个1后面需要的位数，再加上偏移量(127)的二进制形式即为阶码


  //因为x第一个1位移到开头之后的23位为尾数，而尾数之后的8位可能有1，故需进位
  ...
  if((x&0x000001ff) == 0x180) 
    ...
    d = 1;
  frac = ((x&0x7fffffff) >> 8) + d; //尾数为去掉符号位之后向右移8位到达尾数该在的位置
